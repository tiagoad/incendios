/*global L:false, $:false, Spinner:false */
...
$.each(layerToggles, function(key, value) {
	...
	{
		var cookieValue = $.cookie(value.id) == 'true' ? true : false;
...
function pullFireData()
{
	...
	$.getJSON('/fires.json', function(data) {
		...
		var newIdList = Array();
    for(var k in data.fires) {
      if (data.fires.hasOwnProperty(k))
      {
				newIdList.push(k);
      }
    }
		...
		for(var l in fires) {
			if ($.inArray(l, newIdList) == -1) {
				layers.fireLayer.removeLayer(fires[l].fireMarker); // Remove the fire marker from the fire layer
			}
		}
...
(function pullFireDataLoop(){
    pullFireData();
    setTimeout(pullFireDataLoop, 15*1000);
})();

/* Pull fire hazard every 600 seconds */
(function pullHazardDataLoop(){
    pullHazardData();
    setTimeout(pullHazardDataLoop, 600*1000);
})();
